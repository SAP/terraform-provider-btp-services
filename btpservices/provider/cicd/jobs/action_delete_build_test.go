// btpservices/provider/cicd/jobs/action_delete_build_test.go
...
func TestActionDeleteBuild(t *testing.T) {
	t.Parallel()

	t.Run("deletes build successfully", func(t *testing.T) {
		t.Parallel()

		rec, creds := utils.SetupVCR(t, "../fixtures/action_delete_build")
	...
	t.Run("error when build not found (404)", func(t *testing.T) {
		...
		rec, creds := utils.SetupVCR(t, "../fixtures/action_delete_build_not_found")
		defer tfutils.StopQuietly(rec)
...
resource "terraform_data" "trigger" {
  lifecycle {
    action_trigger {
      events  = [after_create]
      actions = [action.btpservice_cicd_delete_build.uut]
    }
  }
}
